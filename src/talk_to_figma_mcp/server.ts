...
// Create Primary Button Set Tool
server.tool(
  "create_primary_button_set",
  "Create a blue primary button set (default/hover/disabled) and place it on the canvas",
  {
    channel: z.string().describe("Channel to join before creating the buttons"),
    x: z.number().default(100).describe("X position for the buttons group"),
    y: z.number().default(100).describe("Y position for the buttons group"),
    label: z.string().optional().describe("Button label (default: Primary)"),
  },
  async ({ channel, x, y, label }: any) => {
    try {
      if (!ws || ws.readyState !== WebSocket.OPEN) {
        connectToFigma();
      ...
      const parent = (await sendCommandToFigma("create_frame", {
        x,
        y,
        width: 360,
        height: 200,
      ...
      async function createButtonVariant(opts: {
        ...
        const frame = (await sendCommandToFigma("create_frame", {
          x: 0,
          y: 0,
          width: 120,
          height: 40,
        ...
        const textNode = (await sendCommandToFigma("create_text", {
          x: 0,
          y: 0,
          text: opts.text,
          fontSize: 14,
          fontWeight: 600,
          fontColor: opts.textColor || { r: 1, g: 1, b: 1, a: 1 },
          name: "Label",
          parentId: frame.id,
        })) as { id: string; name: string };

        return { frameId: frame.id, textId: textNode.id };
      }

      const baseLabel = label || "Primary";

      await createButtonVariant({
        name: "Primary/Default",
        fill: { r: 0.09, g: 0.35, b: 0.88, a: 1 },
        text: baseLabel,
      });

      await createButtonVariant({
        name: "Primary/Hover",
        fill: { r: 0.07, g: 0.3, b: 0.78, a: 1 },
        text: baseLabel,
      });

      await createButtonVariant({
        name: "Primary/Disabled",
        fill: { r: 0.82, g: 0.85, b: 0.9, a: 1 },
        text: baseLabel,
        textColor: { r: 1, g: 1, b: 1, a: 0.6 },
      });

      return {
        content: [
          { type: "text", text: `Created Primary Button Set at (${x}, ${y}) in channel ${channel}` },
        ],
      };
    } catch (error) {
      return {
        content: [
          {
            type: "text",
            text: `Error creating primary button set: ${error instanceof Error ? error.message : String(error)}`,
          },
        ],
      };
    }
