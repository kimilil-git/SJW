...
(() => {
  ...
  function setBlackOrWhiteByBackground(px, py) {
  ...
  function handlePointerMove(e) {
    ...
    // mix-blend-mode 사용으로 자동 대비 (정확한 흑/백 전환 비활성화)
    // setBlackOrWhiteByBackground(tx, ty);
  }
  ...
  function ensureCursorOnTop() {
    ...
    // 모든 섹션의 상태와 관계없이 커서가 최상위에 오도록 보장
    const allSections = document.querySelectorAll('section');
    allSections.forEach(section => {
      const rect = section.getBoundingClientRect();
      // 어떤 섹션이든 화면에 있을 때 커서가 위에 오도록
